...
class Visualization():
    ...
    def wbless(self, save_dir, dbscan=False):
        precision_dict = defaultdict(list)
        recall_dict = defaultdict(list)
        if dbscan:
            paths = [os.path.join(save_dir, x) for x in ['wbless/CLASSICAL_DBSCAN.txt', 'wbless/PROTOTYPE_DBSCAN.txt', 'wbless/PROTOTYPE_KDE_DBSCAN.txt']]
        else:
            #paths = [os.path.join(save_dir, x) for x in ['wbless/CLASSICAL.txt', 'wbless/PROTOTYPE.txt']]
            paths = [os.path.join(save_dir, 'wbless/PROTOTYPE.txt')]
        #methods = ['Classical', 'Prototype (parametric)']
        methods = ['Prototype']
    ...
    def hyperlex(self, save_dir, dbscan=False):
        if dbscan:
            paths = ['hyperlex/CLASSICAL_DBSCAN.txt', 'hyperlex/PROTOTYPE_DBSCAN.txt', 'hyperlex/PROTOTYPE_KDE_DBSCAN.txt']
        else:
            #paths = [os.path.join(save_dir, x) for x in ['wbless/CLASSICAL.txt', 'wbless/PROTOTYPE.txt']]
            paths = [os.path.join(save_dir, 'wbless/PROTOTYPE.txt')]
        #methods = ['Classical', 'Prototype (parametric)']
    ...
    def categorization(self, save_dir, dbscan=False):
        #Classical - accuracy only
        '''y_true = []
        y_pred = []
        if dbscan:
            path = os.path.join(save_dir, 'categorization/CLASSICAL_DBSCAN.txt')
        else:
            path = os.path.join(save_dir, 'categorization/CLASSICAL.txt')
        with open(path) as f:
            for line in f:
        ...
        if dbscan:
            paths = ['categorization/PROTOTYPE_DBSCAN.txt', 'categorization/PROTOTYPE_KDE_DBSCAN.txt']
        else:
            paths = [os.path.join(save_dir, 'categorization/PROTOTYPE.txt')]
        methods = ['Prototype']
        for method, path in zip(methods, paths):
            y_true = []
            y_pred = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--wbless', action='store_true')
    parser.add_argument('--hyperlex', action='store_true')
    parser.add_argument('--categorization', action='store_true')
    parser.add_argument('--save_dir', type=str, default='results/2')
    args = parser.parse_args()

    vis = Visualization(args.model_path)
    if args.wbless:
        vis.wbless(args.save_dir, dbscan=args.dbscan)
    if args.hyperlex:
        vis.hyperlex(args.save_dir, dbscan=args.dbscan)
    if args.categorization:
        vis.categorization(args.save_dir, dbscan=args.dbscan)
    if args.kde:
